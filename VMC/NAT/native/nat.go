...
func main() {
  ...
  // block all connections, but 4001 which is used by etcd. We don't use
  // etcd here, but we still need to push one rule to iptables so it blocks
  // everything. So, just leaving this rule as it is.
  cmd := append([]string{"-A"}, "INPUT", "-p", "tcp", "!", "--sport", "4001",
  ...
  for true {
    ...
    case p := <-packets:
      ...
      if ipv4.SrcIP.String()+":"+strconv.Itoa(int(tcp.SrcPort)) == servAddr {
        ...
        // also if we are sending to more than one service, servAddr will have
        //to iterate to find a matching one
        ...
        // currently we can just use tcp.SrcPort, because we do not remap since
        // we only receive one IP address from kube-proxy who can ensure it
        // does not reuse SrcPorts that is already in use. If expanded to
        // multi-IPs srcs in the future, we'll have to have a mapping of
        // the srcPort from kube-proxy to a new Port
        ...
        if bytes.Compare(payload[len(payload)-15:len(payload)-9], timestampPrefix) == 0 {
          ...
          if err != nil {
            ...
            _, err = statFile.WriteString(fmt.Sprintf("%d %d %d\n",
              timestamp.UnixNano(), time.Now().UnixNano(), len(nat_map)))
            if err != nil {
              fmt.Errorf("could not write to statFile :: %v", err)
            }
          }
        } else {
