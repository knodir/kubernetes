...
# eachRun returns an array with natCapacityVSlatency data. Its index is 
# equal to the capacity of the nat and value is equal to the latency of the 
# packet (when nat had such capacity)
  ...
  for (item in 2:length(cap)) { # skip the first item since it is always zero
    if (retArray[cap[item]] == 0) {
      for (latency_index in 1:length(server_time)) {
        # if difference between nat_time and server time is less than 1 millisec, 
        #we assume those times to be equal and therefore take packet latency 
...
arrFromFirstRun = eachRun("real_data/mac/packet_latency1.txt", 
  "real_data/mac/nat_map_capacity1.txt")
arrFromSecondRun = eachRun("real_data/mac/packet_latency2.txt", 
  "real_data/mac/nat_map_capacity2.txt")
arrFromThirdRun = eachRun("real_data/mac/packet_latency3.txt", 
  "real_data/mac/nat_map_capacity3.txt")

# get the min number of NAT entries for each run and trim array size to be 
# the same, i.e., [0:min]
minLength = min(length(arrFromFirstRun), length(arrFromSecondRun), 
  length(arrFromThirdRun))
...
plot(natCap, laten, type="o", col="blue", xlab="NAT capacity", 
...
title(main="Latency")
# # legend(0,100, c("middlebox 1", "middlebox 2",  "middlebox 3", "middlebox 4"), 
# col=c("blue","red","green", "black"), lty=1:1)
