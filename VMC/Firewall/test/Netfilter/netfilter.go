...
//export go_callback
func go_callback(queueId C.int, data *C.uchar, length C.int, cb *chan NFPacket, newData **C.uchar, newLength *C.uint) Verdict {
	xdata := C.GoBytes(unsafe.Pointer(data), length)
	packet := gopacket.NewPacket(xdata, layers.LayerTypeIPv4, gopacket.Default)
	p := NFPacket{verdictChannel: make(chan VerdictWithData), Packet: packet}
	select {
	case (*cb) <- p:
		v := <-p.verdictChannel
		*newLength = C.uint(v.size)
		*newData = (*C.uchar)(unsafe.Pointer(&v.newData[0]))
		fmt.Printf("Go_callback: %d\n", v.size)
		fmt.Println(v.v)
		fmt.Printf("% X\n", &v.newData[0])
		fmt.Printf("% X\n", v.newData)
		fmt.Printf("Go_callback in c: \n")
